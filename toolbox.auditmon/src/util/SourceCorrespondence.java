...
public class SourceCorrespondence implements Comparable<SourceCorrespondence> {
	...
	private SourceCorrespondence(com.ensoftcorp.atlas.core.index.common.SourceCorrespondence sc) {
		this.sc = sc;
	}

	private SourceCorrespondence(com.ensoftcorp.atlas.core.index.common.SourceCorrespondence sc, String name) {
		this.sc = sc;
		this.name = name;
	}
	...
	public String getRelativeFile() {
		if (relativeFile == null) {
			File baseDirectory = ResourcesPlugin.getWorkspace().getRoot().getProject(getProject()).getLocation().toFile();
			relativeFile = OSUtils.ResourceUtils.getRelativePath(getFile().getAbsolutePath(), baseDirectory.getParent(), "" + File.separatorChar);
		}
		return relativeFile;
	}
	...
	public String getProject() {
		if (project == null) {
			...
			String relativePath = filename.substring(filename.indexOf("+/") + 1);
	...
	public String toString() {
		...
		return "Filename: " + getRelativeFile() + " (" + (lines.contains("-") ? "lines " : "line ") + lines + ")";
	}
	...
	public static Collection<SourceCorrespondence> getSourceCorrespondents(Q q) {
		...
		for (GraphElement node : nodes) {
			...
			if (sc != null) {
				if (name != null) {
					sourceCorrespondents.add(new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) sc, name.toString()));
				} else {
					sourceCorrespondents.add(new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) sc));
				}
			}
			...
			if (scList != null) {
				for (com.ensoftcorp.atlas.core.index.common.SourceCorrespondence scListItem : (List<com.ensoftcorp.atlas.core.index.common.SourceCorrespondence>) scList) {
					if (name != null) {
						sourceCorrespondents.add(new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) scListItem, name.toString()));
					} else {
						sourceCorrespondents.add(new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) scListItem));
					}
				}
			}
		}
	...
	public static SourceCorrespondence getSourceCorrespondent(GraphElement ge) {
		...
		if (sc != null) {
			if (name != null) {
				sourceCorrespondent = new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) sc, name.toString());
			} else {
				sourceCorrespondent = new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) sc);
			}
		}
		...
		if (scList != null) {
			for (com.ensoftcorp.atlas.core.index.common.SourceCorrespondence scListItem : (List<com.ensoftcorp.atlas.core.index.common.SourceCorrespondence>) scList) {
				if (name != null) {
					sourceCorrespondent = new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) scListItem, name.toString());
				} else {
					sourceCorrespondent = new SourceCorrespondence((com.ensoftcorp.atlas.core.index.common.SourceCorrespondence) scListItem);
				}
			}
		}
	...
	private static String summarize(Collection<SourceCorrespondence> allSources, Collection<SourceCorrespondence> methods) {
		...
		for (Entry<String, SortedSet<LineNumber>> entry : filesToLineNumbers.entrySet()) {
			...
			if (!condensedLineNumbers.isEmpty()) {
				result.append(entry.getKey() + " " + condensedLineNumbers.toString().replace("[", "(line" + plurality + " ").replace("]", ")\n"));
			} else {
		...
		for (LineNumber rangeOutside : ranges) {
			for (LineNumber rangeInside : ranges) {
				if (rangeOutside.equals(rangeInside)) {
					...
					String[] startEndLinesInside = rangeInside.toString().split("-");
					int startInside = Integer.parseInt(startEndLinesInside[0]);
					int endInside = Integer.parseInt(startEndLinesInside[1]);
					String[] startEndLinesOutside = rangeOutside.toString().split("-");
					int startOutside = Integer.parseInt(startEndLinesOutside[0]);
			...
			for (LineNumber reference : condensedLines) {
				if (!base.lines.equals(reference.lines)) {
					...
					if (base.isSingle()) {
						...
						if (base.getEnd() == reference.getStart() || base.getEnd() + 1 == reference.getStart()) {
