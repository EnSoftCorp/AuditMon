	...
	public void createPartControl(Composite parent) {
			...
			public void modifyText(ModifyEvent e) {
				if(interestThresholdText.getText().equals("") || interestThresholdText.getText().equals("-") || interestThresholdText.getText().equals("-.")){
					...
					try {
						...
						if(d > 0){
							unsynchronize(sessionText, startStopSynchronizeAction);
							DisplayUtils.showMessage("Invalid Input: Interest threshold should be less than or equal to zero.");
						} else {
							refreshTable(sessionText, granularityComboBox, decayRateText, interestIncrementText, interestThresholdText);
						}
					} catch (Exception ex){
						DisplayUtils.showError(ex, "Invalid Input: Interest threshold must be an integer or decimal number.");
			...
			public void modifyText(ModifyEvent e) {
				if(decayRateText.getText().equals("") || decayRateText.getText().equals("-") || decayRateText.getText().equals("-.")){
					...
					try {
						...
						if(d < 0){
							unsynchronize(sessionText, startStopSynchronizeAction);
							DisplayUtils.showMessage("Invalid Input: Decay rate should be greater than or equal to zero.");
						} else {
							refreshTable(sessionText, granularityComboBox, decayRateText, interestIncrementText, interestThresholdText);
						}
					} catch (Exception ex){
						DisplayUtils.showError(ex, "Invalid Input: Decay rate must be an integer or decimal number.");
			...
			public void modifyText(ModifyEvent e) {
				if(interestIncrementText.getText().equals("") || interestIncrementText.getText().equals("-") || interestIncrementText.getText().equals("-.")){
					...
					try {
						...
						if(d <= 0){
							unsynchronize(sessionText, startStopSynchronizeAction);
							DisplayUtils.showMessage("Invalid Input: Interest increment should be greater than zero.");
						} else {
							refreshTable(sessionText, granularityComboBox, decayRateText, interestIncrementText, interestThresholdText);
						}
					} catch (Exception ex){
						DisplayUtils.showError(ex, "Invalid Input: Interest increment must be an integer or decimal number.");
		...
		final Action doubleClickAction = new Action() {
			public void run() {
				...
				if(obj instanceof GraphElement){
					DisplayUtils.show(Common.toQ(Common.toGraph((GraphElement)obj)), null, true, ((GraphElement)obj).attr().get(Node.NAME).toString());
				}
			}
		};
